...
console.log('=== Starting JavaScript Masterclass Source Audit ===\n');
...
const indexFile = path.join(__dirname, '..', 'src', 'features', 'roadmap', 'data', 'javascript', 'index.ts');
const serviceFile = path.join(__dirname, '..', 'src', 'features', 'roadmap', 'services', 'javascriptCourse.service.ts');
const screenFile = path.join(__dirname, '..', 'src', 'features', 'roadmap', 'components', 'JavaScriptCourseScreen.tsx');
...
assert(fs.existsSync(begFile), 'javascriptBeginner.data.ts exists');
assert(fs.existsSync(intFile), 'javascriptIntermediate.data.ts exists');
assert(fs.existsSync(advFile), 'javascriptAdvanced.data.ts exists');
assert(fs.existsSync(indexFile), 'javascript/index.ts exists');
assert(fs.existsSync(serviceFile), 'javascriptCourse.service.ts exists');
assert(fs.existsSync(screenFile), 'JavaScriptCourseScreen.tsx exists');
...
const begTasks = begData.JAVASCRIPT_BEGINNER_TASKS;
const intTasks = intData.JAVASCRIPT_INTERMEDIATE_TASKS;
const advTasks = advData.JAVASCRIPT_ADVANCED_TASKS;

const begMCQs = begData.JAVASCRIPT_BEGINNER_FINAL_ASSESSMENT;
const intMCQs = intData.JAVASCRIPT_INTERMEDIATE_FINAL_ASSESSMENT;
const advMCQs = advData.JAVASCRIPT_ADVANCED_FINAL_ASSESSMENT;
...
assert(serviceContent.includes("const STORAGE_KEY = 'skillora_javascript_course_progress';"), 'Service uses exact storage key "skillora_javascript_course_progress"');
...
assert(appContent.includes("path=\"/courses/javascript\""), 'App.tsx contains primary route /courses/javascript');
assert(appContent.includes("path=\"/courses/javascript-masterclass\""), 'App.tsx contains alias route /courses/javascript-masterclass');
assert(appContent.includes("path=\"/courses/javascript/beginner\""), 'App.tsx contains subroute /courses/javascript/beginner');
assert(appContent.includes("path=\"/courses/javascript/intermediate\""), 'App.tsx contains subroute /courses/javascript/intermediate');
assert(appContent.includes("path=\"/courses/javascript/advanced\""), 'App.tsx contains subroute /courses/javascript/advanced');
assert(appContent.includes("JavaScriptCourseScreen"), 'App.tsx imports and renders JavaScriptCourseScreen');

const catalogContent = fs.readFileSync(catalogFile, 'utf-8');
assert(catalogContent.includes("id: 'javascript'"), 'CourseCatalogScreen contains javascript course card');
assert(catalogContent.includes("JavaScriptCourseService"), 'CourseCatalogScreen uses JavaScriptCourseService');

const searchContent = fs.readFileSync(searchFile, 'utf-8');
assert(searchContent.includes("course_javascript_masterclass"), 'search.service.ts contains javascript index');
...
const forbiddenStrings = ['placeholder course', 'placeholder module', 'coming soon', 'content will be added later', 'research this topic', 'research more on your own', 'try something creative', 'todo'];
...
console.log('\n==================================================');
if (errorCount === 0) {
  console.log('🏆 JAVASCRIPT MASTERCLASS AUDIT: 100% PASSED — READY FOR PRODUCTION!');
  process.exit(0);
} else {
  console.error(`💥 JAVASCRIPT MASTERCLASS AUDIT: FAILED WITH ${errorCount} ERRORS.`);
  process.exit(1);
}
