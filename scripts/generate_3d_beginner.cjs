...
const targetFile = path.join(__dirname, '..', 'src', 'features', 'roadmap', 'data', '3d-animation', 'threeDAnimationBeginner.data.ts');
...
// Helper to generate rich modules
const modules = [
  {
    id: '3d-beg-01',
    orderIndex: 1,
...
print(f"Total Armatures (Rigs): {len(bpy.data.armatures)}")`,
            ...
            lineByLine: [
              { line: 'import bpy', explanation: 'Imports the core Blender Python interface module.' },
              { line: 'scene = bpy.context.scene', explanation: 'Accesses the active viewport scene and render configuration settings.' },
              { line: 'print(f"Render Engine: {scene.render.engine}")', explanation: 'Outputs whether the scene is utilizing CYCLES path tracing or BLENDER_EEVEE_NEXT.' },
              { line: 'print(f"Output Resolution: ...")', explanation: 'Verifies target delivery resolution (e.g. 1920x1080) and playback frame rate (24 fps).' }
            ]
          }
        ],
...
// We will construct the rest of the 12 beginner modules programmatically with full rich depth
const additionalBeginnerModules = [
  {
    id: '3d-beg-03',
    orderIndex: 3,
...
// Combine all 12 beginner tasks
const allBeginnerTasks = [];

// Push first 2 detailed modules
allBeginnerTasks.push(...modules.map(m => ({
  id: m.id,
  orderIndex: m.orderIndex,
  title: m.title,
  description: m.description,
  status: m.orderIndex === 1 ? 'in_progress' : 'locked',
  requiresQuiz: true,
  xpReward: 150,
  level: 'beginner',
  resources: [
  ...
  lessonContent: {
    ...
    analogyHero: m.analogyHero,
    objectives: m.objectives,
    estimatedTime: m.estimatedTime,
    syntaxGuide: m.syntaxGuide,
    sections: m.sections,
...
// Push modules 3 to 12
additionalBeginnerModules.forEach(m => {
  allBeginnerTasks.push({
    id: m.id,
    orderIndex: m.orderIndex,
    title: m.title,
    description: m.description,
    ...
    lessonContent: {
      ...
      sections: [
        {
          id: `sec-${m.id}-1`,
          title: m.sec1?.title || 'Core Principles and Practical Applications',
          difficulty: 'Beginner',
          content: m.sec1?.content || 'In-depth exploration of core 3D principles, mathematical spatial representations, and practical workflow steps required for animation in Blender.',
          codeSnippets: [
            {
              language: 'python',
              filename: `${m.id.replace(/-/g, '_')}_workflow.py`,
              title: 'Blender Automation Snippet',
              code: `import bpy\n\n# Configure scene parameters for ${m.title}\nscene = bpy.context.scene\nscene.render.fps = 24\nprint("Configured 24fps cinematic animation standards.")`,
          ...
          miniPractice: {
            question: `What is the primary best practice emphasized in ${m.title}?`,
            options: [
        ...
        {
          id: `sec-${m.id}-2`,
          title: m.sec2?.title || 'Step-by-Step Production Execution',
          difficulty: 'Beginner',
          content: m.sec2?.content || 'Step-by-step guidance on setting up, refining, and validating parameters in Blender to achieve professional visual quality and believable motion dynamics.',
          codeSnippets: [
            {
              language: 'text',
        ...
        {
          id: `sec-${m.id}-3`,
          title: m.sec3?.title || 'Troubleshooting and Industry Best Practices',
          difficulty: 'Beginner',
          content: m.sec3?.content || 'Professional troubleshooting techniques for diagnosing unexpected geometry stretching, abnormal light clipping, gimbal lock, and timing hitches.',
          codeSnippets: [
            {
              language: 'text',
...
Camera Clipping Interior| Near Clip too high       | Camera Settings -> Clip Start = 0.01m`,
      ...
      summary: `You have completed ${m.title}, acquiring foundational competencies and technical confidence in Blender 3D.`
    }
  });
});

// Add Beginner Project
allBeginnerTasks.push({
  id: '3d-beg-p1',
  orderIndex: 13,
...
allBeginnerTasks.push({
  ...
  questions: beginnerMCQs
});

// Write to TypeScript file
const fileContent = `import { Task, QuizQuestion } from '../../types/roadmap.types';

export const THREE_D_BEGINNER_FINAL_ASSESSMENT: QuizQuestion[] = ${JSON.stringify(beginnerMCQs, null, 2)};

export const THREE_D_BEGINNER_TASKS: Task[] = ${JSON.stringify(allBeginnerTasks, null, 2)};
`;

fs.writeFileSync(targetFile, fileContent, 'utf-8');
console.log(`Successfully generated ${targetFile} with ${allBeginnerTasks.length} tasks and ${beginnerMCQs.length} MCQs.`);
