...
console.log('=== Starting Google Ads Masterclass Source Audit ===\n');
...
const indexFile = path.join(__dirname, '..', 'src', 'features', 'roadmap', 'data', 'google-ads', 'index.ts');
const serviceFile = path.join(__dirname, '..', 'src', 'features', 'roadmap', 'services', 'googleAdsCourse.service.ts');
const screenFile = path.join(__dirname, '..', 'src', 'features', 'roadmap', 'components', 'GoogleAdsCourseScreen.tsx');
...
assert(fs.existsSync(begFile), 'googleAdsBeginner.data.ts exists');
assert(fs.existsSync(intFile), 'googleAdsIntermediate.data.ts exists');
assert(fs.existsSync(advFile), 'googleAdsAdvanced.data.ts exists');
assert(fs.existsSync(indexFile), 'google-ads/index.ts exists');
assert(fs.existsSync(serviceFile), 'googleAdsCourse.service.ts exists');
assert(fs.existsSync(screenFile), 'GoogleAdsCourseScreen.tsx exists');
...
const begTasks = begData.GOOGLE_ADS_BEGINNER_TASKS;
const intTasks = intData.GOOGLE_ADS_INTERMEDIATE_TASKS;
const advTasks = advData.GOOGLE_ADS_ADVANCED_TASKS;

const begMCQs = begData.GOOGLE_ADS_BEGINNER_FINAL_ASSESSMENT;
const intMCQs = intData.GOOGLE_ADS_INTERMEDIATE_FINAL_ASSESSMENT;
const advMCQs = advData.GOOGLE_ADS_ADVANCED_FINAL_ASSESSMENT;
...
assert(serviceContent.includes("const STORAGE_KEY = 'skillora_google_ads_course_progress';"), 'Service uses exact storage key "skillora_google_ads_course_progress"');
...
assert(appContent.includes("path=\"/courses/google-ads\""), 'App.tsx contains primary route /courses/google-ads');
assert(appContent.includes("path=\"/courses/google-ads-masterclass\""), 'App.tsx contains alias route /courses/google-ads-masterclass');
assert(appContent.includes("path=\"/courses/google-ads/beginner\""), 'App.tsx contains subroute /courses/google-ads/beginner');
assert(appContent.includes("path=\"/courses/google-ads/intermediate\""), 'App.tsx contains subroute /courses/google-ads/intermediate');
assert(appContent.includes("path=\"/courses/google-ads/advanced\""), 'App.tsx contains subroute /courses/google-ads/advanced');
assert(appContent.includes("GoogleAdsCourseScreen"), 'App.tsx imports and renders GoogleAdsCourseScreen');

const catalogContent = fs.readFileSync(catalogFile, 'utf-8');
assert(catalogContent.includes("id: 'google-ads'"), 'CourseCatalogScreen contains google-ads course card');
assert(catalogContent.includes("GoogleAdsCourseService"), 'CourseCatalogScreen uses GoogleAdsCourseService');

const searchContent = fs.readFileSync(searchFile, 'utf-8');
assert(searchContent.includes("course_google_ads_masterclass"), 'search.service.ts contains google-ads index');
...
console.log('\n==================================================');
if (errorCount === 0) {
  console.log('🏆 GOOGLE ADS MASTERCLASS AUDIT: 100% PASSED — READY FOR PRODUCTION!');
  process.exit(0);
} else {
  console.error(`💥 GOOGLE ADS MASTERCLASS AUDIT: FAILED WITH ${errorCount} ERRORS.`);
  process.exit(1);
}
