...
import { INITIAL_SHOPIFY_COURSE_STATE } from '../data/shopify';

const STORAGE_KEY = 'skillora_shopify_course_progress';
...
export class ShopifyCourseService {
  /**
   * Load current user's Shopify Course progress state from localStorage
   */
  static getCourseState(): CourseProgressState {
    try {
      ...
      console.warn('Failed to load Shopify course state from localStorage:', err);
    }
    return JSON.parse(JSON.stringify(INITIAL_SHOPIFY_COURSE_STATE));
  }

  /**
   * Save current Shopify Course progress state
   */
  static saveCourseState(state: CourseProgressState): void {
    try {
      setStorageItem(STORAGE_KEY, JSON.stringify(state));
    } catch (err) {
      console.warn('Failed to save Shopify course state to localStorage:', err);
    }
  }
  ...
  static resetCourseState(): CourseProgressState {
    const pristine = JSON.parse(JSON.stringify(INITIAL_SHOPIFY_COURSE_STATE));
  ...
  private static normalizeCourseState(state: CourseProgressState): CourseProgressState {
    const base = JSON.parse(JSON.stringify(INITIAL_SHOPIFY_COURSE_STATE)) as CourseProgressState;
  ...
  static checkLevelPrerequisites(targetLevel: CourseLevel, state: CourseProgressState = this.getCourseState()): {
    ...
    if (targetLevel === 'intermediate') {
      ...
      if (!allModulesDone || !assessmentPassed) {
        ...
        if (!allModulesDone) {
          ...
          reason += ` Complete all 12 Beginner modules & project (${completedCount}/${nonAssessmentTasks.length} done).`;
        }
        if (!assessmentPassed) {
    ...
    if (targetLevel === 'advanced') {
      ...
      if (!allModulesDone || !assessmentPassed) {
        ...
        if (!allModulesDone) {
          const completedCount = nonAssessmentTasks.filter(t => t.status === 'completed').length;
          reason += ` Complete all 14 Intermediate modules & project (${completedCount}/${nonAssessmentTasks.length} done).`;
        }
        if (!assessmentPassed) {
  ...
  static isEligibleForMasterCertificate(state: CourseProgressState = this.getCourseState()): {
    ...
    if (!advAssessmentPassed) {
      reason += `Score at least 35/40 on the Shopify Liquid Advanced Master Assessment (Current: ${advScore}/40). `;
    }
  ...
  static completeTask(level: CourseLevel, taskId: string): CourseProgressState {
    ...
    // If all tasks in level are done and assessment passed, mark level completed
    const nonAssessTasks = lvl.tasks.filter(t => !t.isFinalAssessment);
    const allModulesDone = nonAssessTasks.every(t => t.status === 'completed');
    if (allModulesDone && lvl.assessmentPassed) {
      lvl.status = 'completed';
      
      // If beginner completed, unlock intermediate
      if (level === 'beginner' && (lvl.assessmentScore ?? 0) >= 15) {
        if (state.levels.intermediate.status === 'locked') {
      ...
      } else if (level === 'intermediate' && (lvl.assessmentScore ?? 0) >= 25) {
        if (state.levels.advanced.status === 'locked') {
  ...
  /**
   * Submit assessment answers, grade score, compute weak areas, and unlock next level if passed
   */
  static evaluateAssessment(
    ...
    questions.forEach(q => {
      const topic = q.topic || 'Liquid Core Knowledge';
      if (!topicStats[topic]) {
