...
import { INITIAL_YOUTUBE_ADS_COURSE_STATE } from '../data/youtube-ads';

const STORAGE_KEY = 'skillora_youtube_ads_course_progress';
...
export class YouTubeAdsCourseService {
  /**
   * Load current user's YouTube Ads Course progress state from localStorage
   */
  static getCourseState(): CourseProgressState {
    try {
      ...
      console.warn('Failed to load YouTube Ads course state from localStorage:', err);
    }
    return JSON.parse(JSON.stringify(INITIAL_YOUTUBE_ADS_COURSE_STATE));
  }

  /**
   * Save current YouTube Ads Course progress state
   */
  static saveCourseState(state: CourseProgressState): void {
    try {
      setStorageItem(STORAGE_KEY, JSON.stringify(state));
    } catch (err) {
      console.warn('Failed to save YouTube Ads course state to localStorage:', err);
    }
  }
  ...
  static resetCourseState(): CourseProgressState {
    const pristine = JSON.parse(JSON.stringify(INITIAL_YOUTUBE_ADS_COURSE_STATE));
  ...
  private static normalizeCourseState(state: CourseProgressState): CourseProgressState {
    const base = JSON.parse(JSON.stringify(INITIAL_YOUTUBE_ADS_COURSE_STATE)) as CourseProgressState;
  ...
  static checkLevelPrerequisites(
    ...
    if (targetLevel === 'intermediate') {
      ...
      if (!allBegTasksDone || !begExamPassed) {
        return {
          canUnlock: false,
          reason: 'You must complete all 12 Beginner modules, the Video Action Campaign Launch Blueprint Project, and pass the Beginner Assessment with at least 15/20 before unlocking Intermediate Level.'
        };
      }
      return { canUnlock: true };
    }
    ...
    if (targetLevel === 'advanced') {
      ...
      if (!allInterTasksDone || !interExamPassed) {
        return {
          canUnlock: false,
          reason: 'You must complete all 14 Intermediate modules, the Full-Funnel Multi-Stage YouTube Ads Project, and pass the Intermediate Assessment with at least 25/30 before unlocking Advanced Level.'
        };
      }
      return { canUnlock: true };
    }
  ...
  /**
   * Check if user is eligible to claim the Master YouTube Ads Certificate
   */
  static isEligibleForMasterCertificate(currentState?: CourseProgressState): {
    ...
    if (!begTasksDone || begScore < beg.requiredScore) {
      unmetRequirements.push('Complete all 12 Beginner modules, VAC Launch Project, and pass Beginner Assessment (>= 15/20).');
    }
    ...
    if (!interTasksDone || interScore < inter.requiredScore) {
      unmetRequirements.push('Complete all 14 Intermediate modules, Full-Funnel Architecture Project, and pass Intermediate Assessment (>= 25/30).');
    }
    ...
    if (!advTasksDone) {
      unmetRequirements.push('Complete all 14 Advanced modules and both Capstone Projects (Enterprise Ad Ops & Incrementality/AI Bidding).');
    }
