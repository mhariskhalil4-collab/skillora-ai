...
import { INITIAL_ENGLISH_EMAIL_WRITING_COURSE_STATE } from '../data/english-email-writing';

const STORAGE_KEY = 'skillora_english_email_writing_course_progress';
...
export class EnglishEmailWritingCourseService {
  /**
   * Load current user's English Email Writing Course progress state from localStorage
   */
  static getCourseState(): CourseProgressState {
    try {
      ...
      console.warn('Failed to load English Email Writing course state from localStorage:', err);
    }
    return JSON.parse(JSON.stringify(INITIAL_ENGLISH_EMAIL_WRITING_COURSE_STATE));
  }

  /**
   * Save current English Email Writing Course progress state
   */
  static saveCourseState(state: CourseProgressState): void {
    try {
      setStorageItem(STORAGE_KEY, JSON.stringify(state));
    } catch (err) {
      console.warn('Failed to save English Email Writing course state to localStorage:', err);
    }
  }
  ...
  static resetCourseState(): CourseProgressState {
    const pristine = JSON.parse(JSON.stringify(INITIAL_ENGLISH_EMAIL_WRITING_COURSE_STATE));
  ...
  static normalizeCourseState(state: CourseProgressState): CourseProgressState {
    const base = JSON.parse(JSON.stringify(INITIAL_ENGLISH_EMAIL_WRITING_COURSE_STATE)) as CourseProgressState;
  ...
  static submitAssessment(
    ...
    assessmentQuestions.forEach((q) => {
      const topic = q.topic || 'General English Email Writing';
  ...
  static checkLevelPrerequisites(
    ...
    if (targetLevel === 'intermediate') {
      ...
      if (!allBegTasksDone || !begExamPassed) {
        return {
          canUnlock: false,
          reason: 'You must complete all 12 Beginner modules, the Beginner Workplace Email Project, and pass the Beginner Assessment with at least 15/20 before unlocking Intermediate Level.'
        };
      }
      return { canUnlock: true };
    }
    ...
    if (targetLevel === 'advanced') {
      ...
      if (!allInterTasksDone || !interExamPassed) {
        return {
          canUnlock: false,
          reason: 'You must complete all 14 Intermediate modules, the Commercial & Workplace Portfolio Project, and pass the Intermediate Assessment with at least 25/30 before unlocking Advanced Level.'
        };
      }
      return { canUnlock: true };
    }
  ...
  /**
   * Validate if the user is eligible for the Master Certificate
   */
  static isEligibleForMasterCertificate(currentState?: CourseProgressState): {
    ...
    if (!advModules.every(t => t.status === 'completed')) {
      unmetRequirements.push('Complete all Level 3 Advanced modules & both Strategic Capstone Projects');
    }
