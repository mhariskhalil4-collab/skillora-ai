...
import { INITIAL_SHOPIFY_DEV_COURSE_STATE } from '../data/shopify-developer';

const STORAGE_KEY = 'skillora_shopify_developer_course_progress';
...
export class ShopifyDeveloperCourseService {
  /**
   * Load current user's Shopify Developer Course progress state from localStorage
   */
  static getCourseState(): CourseProgressState {
    try {
      ...
      console.warn('Failed to load Shopify Developer course state from localStorage:', err);
    }
    return JSON.parse(JSON.stringify(INITIAL_SHOPIFY_DEV_COURSE_STATE));
  }

  /**
   * Save current Shopify Developer Course progress state
   */
  static saveCourseState(state: CourseProgressState): void {
    try {
      setStorageItem(STORAGE_KEY, JSON.stringify(state));
    } catch (err) {
      console.warn('Failed to save Shopify Developer course state to localStorage:', err);
    }
  }
  ...
  static resetCourseState(): CourseProgressState {
    const pristine = JSON.parse(JSON.stringify(INITIAL_SHOPIFY_DEV_COURSE_STATE));
  ...
  static normalizeCourseState(state: CourseProgressState): CourseProgressState {
    const base = JSON.parse(JSON.stringify(INITIAL_SHOPIFY_DEV_COURSE_STATE)) as CourseProgressState;
  ...
  static submitAssessment(
    ...
    assessmentQuestions.forEach((q) => {
      const topic = q.topic || 'General Shopify Development';
  ...
  static checkLevelPrerequisites(
    ...
    if (targetLevel === 'intermediate') {
      ...
      if (!allBegTasksDone || !begExamPassed) {
        return {
          canUnlock: false,
          reason: 'You must complete all 12 Beginner modules, the Custom Shopify Theme Feature Project, and pass the Beginner Assessment with at least 15/20 before unlocking Intermediate Level.'
        };
      }
      return { canUnlock: true };
    }
    ...
    if (targetLevel === 'advanced') {
      ...
      if (!allInterTasksDone || !interExamPassed) {
        return {
          canUnlock: false,
          reason: 'You must complete all 14 Intermediate modules, the Shopify Custom App Project, and pass the Intermediate Assessment with at least 25/30 before unlocking Advanced Level.'
        };
      }
      return { canUnlock: true };
    }
