...
import { INITIAL_PYTHON_COURSE_STATE } from '../data/python';

const STORAGE_KEY = 'skillora_python_course_progress';

export class PythonCourseService {
  /**
   * Load current user's Python Course progress state from localStorage and/or Supabase
   */
  static getCourseState(): CourseProgressState {
    try {
      ...
      if (saved) {
        ...
        // Merge with initial state in case new tasks/modules were added
        return this.normalizeCourseState(parsed);
      }
    } catch (err) {
      console.warn('Failed to load Python course state from localStorage:', err);
    }
    return JSON.parse(JSON.stringify(INITIAL_PYTHON_COURSE_STATE));
  }

  /**
   * Save current Python Course progress state
   */
  static saveCourseState(state: CourseProgressState): void {
    try {
      localStorage.setItem(STORAGE_KEY, JSON.stringify(state));
    } catch (err) {
      console.warn('Failed to save Python course state to localStorage:', err);
    }
  }
  ...
  private static normalizeCourseState(state: CourseProgressState): CourseProgressState {
    const base = JSON.parse(JSON.stringify(INITIAL_PYTHON_COURSE_STATE)) as CourseProgressState;
  ...
  static checkLevelPrerequisites(targetLevel: CourseLevel, state: CourseProgressState = this.getCourseState()): {
    ...
    if (targetLevel === 'intermediate') {
      ...
      if (!allModulesDone || !assessmentPassed) {
        ...
        if (!allModulesDone) {
          ...
          reason += ` Complete all 14 Beginner modules (${completedCount}/14 done).`;
        }
        if (!assessmentPassed) {
    ...
    if (targetLevel === 'advanced') {
      ...
      if (!allModulesDone || !assessmentPassed) {
        ...
        if (!allModulesDone) {
          const completedCount = nonAssessmentTasks.filter(t => t.status === 'completed').length;
          reason += ` Complete all 14 Intermediate modules (${completedCount}/14 done).`;
        }
        if (!assessmentPassed) {
  ...
  static isEligibleForMasterCertificate(state: CourseProgressState = this.getCourseState()): {
    ...
    if (!advAssessmentPassed) {
      reason += `Score at least 35/40 on the Python Advanced Comprehensive Master Assessment (Current: ${advScore}/40). `;
    }
