...
import { INITIAL_INSTAGRAM_ADS_COURSE_STATE } from '../data/instagram-ads';

const STORAGE_KEY = 'skillora_instagram_ads_course_progress';
...
export class InstagramAdsCourseService {
  /**
   * Load current user's Instagram Ads Course progress state from localStorage
   */
  static getCourseState(): CourseProgressState {
    try {
      ...
      console.warn('Failed to load Instagram Ads course state from localStorage:', err);
    }
    return JSON.parse(JSON.stringify(INITIAL_INSTAGRAM_ADS_COURSE_STATE));
  }

  /**
   * Save current Instagram Ads Course progress state
   */
  static saveCourseState(state: CourseProgressState): void {
    try {
      setStorageItem(STORAGE_KEY, JSON.stringify(state));
    } catch (err) {
      console.warn('Failed to save Instagram Ads course state to localStorage:', err);
    }
  }
  ...
  static resetCourseState(): CourseProgressState {
    const pristine = JSON.parse(JSON.stringify(INITIAL_INSTAGRAM_ADS_COURSE_STATE));
  ...
  private static normalizeCourseState(state: CourseProgressState): CourseProgressState {
    const base = JSON.parse(JSON.stringify(INITIAL_INSTAGRAM_ADS_COURSE_STATE)) as CourseProgressState;
  ...
  static submitAssessment(
    ...
    if (passed) {
      const allModulesDone = lvl.tasks.filter(t => !t.isFinalAssessment).every(t => t.status === 'completed');
      if (allModulesDone) {
  ...
  static checkLevelPrerequisites(
    ...
    if (targetLevel === 'intermediate') {
      ...
      if (!allBegTasksDone || !begExamPassed) {
        return {
          canUnlock: false,
          reason: 'You must complete all 12 Beginner modules, the Complete Instagram Direct-Response E-Commerce Launch Project, and pass the Beginner Assessment with at least 15/20 before unlocking Intermediate Level.'
        };
      }
      return { canUnlock: true };
    }
    ...
    if (targetLevel === 'advanced') {
      ...
      if (!allInterTasksDone || !interExamPassed) {
        return {
          canUnlock: false,
          reason: 'You must complete all 14 Intermediate modules, the Full-Funnel Instagram Growth & Omnichannel Scaling Project, and pass the Intermediate Assessment with at least 25/30 before unlocking Advanced Level.'
        };
      }
      return { canUnlock: true };
    }
  ...
  /**
   * Check if user is eligible to claim the Master Instagram Ads Certificate
   */
  static isEligibleForMasterCertificate(currentState?: CourseProgressState): {
    ...
    if (!begTasksDone || begScore < beg.requiredScore) {
      unmetRequirements.push('Complete all 12 Beginner modules, DR E-Commerce Launch Project, and pass Beginner Assessment (>= 15/20).');
    }
    ...
    if (!interTasksDone || interScore < inter.requiredScore) {
      unmetRequirements.push('Complete all 14 Intermediate modules, Full-Funnel Scaling Project, and pass Intermediate Assessment (>= 25/30).');
    }
    ...
    if (!advTasksDone) {
      unmetRequirements.push('Complete all 14 Advanced modules and both Capstone Projects (Enterprise ASC/CAPI & Marketing Mix Modeling).');
    }
