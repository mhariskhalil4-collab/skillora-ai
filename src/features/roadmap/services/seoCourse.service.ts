...
import { INITIAL_SEO_COURSE_STATE } from '../data/seo';

const STORAGE_KEY = 'skillora_seo_course_progress';
...
export class SeoCourseService {
  /**
   * Load current user's SEO Course progress state from localStorage
   */
  static getCourseState(): CourseProgressState {
    try {
      ...
      console.warn('Failed to load SEO course state from localStorage:', err);
    }
    return JSON.parse(JSON.stringify(INITIAL_SEO_COURSE_STATE));
  }

  /**
   * Save current SEO Course progress state
   */
  static saveCourseState(state: CourseProgressState): void {
    try {
      setStorageItem(STORAGE_KEY, JSON.stringify(state));
    } catch (err) {
      console.warn('Failed to save SEO course state to localStorage:', err);
    }
  }
  ...
  static resetCourseState(): CourseProgressState {
    const pristine = JSON.parse(JSON.stringify(INITIAL_SEO_COURSE_STATE));
  ...
  private static normalizeCourseState(state: CourseProgressState): CourseProgressState {
    const base = JSON.parse(JSON.stringify(INITIAL_SEO_COURSE_STATE)) as CourseProgressState;
  ...
  static isEligibleForMasterCertificate(state: CourseProgressState = this.getCourseState()): {
    ...
    if (!advAssessmentPassed) {
      reason += `Score at least 35/40 on the SEO Advanced Master Assessment (Current: ${advScore}/40). `;
    }
  ...
  static submitAssessment(
    ...
    questions.forEach(q => {
      const topic = q.topic || 'SEO Core Competency';
      if (!topicStats[topic]) {
  ...
  /**
   * Get complete diagnostics of user's SEO course progress
   */
