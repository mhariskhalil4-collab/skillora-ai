...
import { INITIAL_FACEBOOK_ADS_COURSE_STATE } from '../data/facebook-ads';

const STORAGE_KEY = 'skillora_facebook_ads_course_progress';
...
export class FacebookAdsCourseService {
  /**
   * Load current user's Facebook Ads Course progress state from localStorage
   */
  static getCourseState(): CourseProgressState {
    try {
      ...
      console.warn('Failed to load Facebook Ads course state from localStorage:', err);
    }
    return JSON.parse(JSON.stringify(INITIAL_FACEBOOK_ADS_COURSE_STATE));
  }

  /**
   * Save current Facebook Ads Course progress state
   */
  static saveCourseState(state: CourseProgressState): void {
    try {
      setStorageItem(STORAGE_KEY, JSON.stringify(state));
    } catch (err) {
      console.warn('Failed to save Facebook Ads course state to localStorage:', err);
    }
  }
  ...
  static resetCourseState(): CourseProgressState {
    const pristine = JSON.parse(JSON.stringify(INITIAL_FACEBOOK_ADS_COURSE_STATE));
  ...
  private static normalizeCourseState(state: CourseProgressState): CourseProgressState {
    const base = JSON.parse(JSON.stringify(INITIAL_FACEBOOK_ADS_COURSE_STATE)) as CourseProgressState;
  ...
  static checkLevelPrerequisites(
    ...
    if (targetLevel === 'intermediate') {
      ...
      if (!allBegTasksDone || !begExamPassed) {
        return {
          canUnlock: false,
          reason: 'You must complete all 12 Beginner modules, the Local Business Lead Generation Project, and pass the Beginner Assessment with at least 15/20 before unlocking Intermediate Level.'
        };
      }
      return { canUnlock: true };
    }
    ...
    if (targetLevel === 'advanced') {
      ...
      if (!allInterTasksDone || !interExamPassed) {
        return {
          canUnlock: false,
          reason: 'You must complete all 14 Intermediate modules, the E-commerce Sales Growth Project, and pass the Intermediate Assessment with at least 25/30 before unlocking Advanced Level.'
        };
      }
      return { canUnlock: true };
    }
  ...
  /**
   * Check if user is eligible to claim the Master Facebook Ads Certificate
   */
  static isEligibleForMasterCertificate(currentState?: CourseProgressState): {
    ...
    if (!begTasksDone || begScore < beg.requiredScore) {
      unmetRequirements.push('Complete all 12 Beginner modules, Local Lead Gen Project, and pass Beginner Assessment (>= 15/20).');
    }
    ...
    if (!interTasksDone || interScore < inter.requiredScore) {
      unmetRequirements.push('Complete all 14 Intermediate modules, E-commerce Growth Project, and pass Intermediate Assessment (>= 25/30).');
    }
    ...
    if (!advTasksDone) {
      unmetRequirements.push('Complete all 14 Advanced modules and both Capstone Projects (Omnichannel Growth System & Recovery Case Study).');
    }
