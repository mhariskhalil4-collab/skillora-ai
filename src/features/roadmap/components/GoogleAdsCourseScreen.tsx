...
export const GoogleAdsCourseScreen: React.FC = () => {
  const [courseState, setCourseState] = useState<CourseProgressState>(() => GoogleAdsCourseService.getCourseState());
  ...
  const loadState = useCallback(() => {
    const state = GoogleAdsCourseService.getCourseState();
    ...
    if (levelParam && ['beginner', 'intermediate', 'advanced'].includes(levelParam)) {
      const check = GoogleAdsCourseService.checkLevelPrerequisites(levelParam, state);
      if (!check.canUnlock) {
      ...
      const activeCheck = GoogleAdsCourseService.checkLevelPrerequisites(state.currentLevel, state);
  ...
  const certEligibility = GoogleAdsCourseService.isEligibleForMasterCertificate(courseState);
  ...
  const handleLevelSelect = (level: CourseLevel) => {
    const res = GoogleAdsCourseService.switchLevel(level);
    if (res.success) {
  ...
  const handleTaskCompleted = async (taskId: string) => {
    const updatedState = GoogleAdsCourseService.completeTask(activeLevel, taskId);
    setCourseState(updatedState);

    // Auto-check if completed master level to issue credential
    if (user?.id) {
      const eligibility = GoogleAdsCourseService.isEligibleForMasterCertificate(updatedState);
      if (eligibility.isEligible) {
        try {
          await CertificateService.getOrCreateCertificate(
            user.id,
            'Google Ads Masterclass — Performance Marketing & Paid Search Specialist',
            '3d-gold'
          );
        } catch (e) {
  ...
  const handleClaimCertificate = async () => {
    ...
    try {
      ...
      setCertClaimedToast('🎉 Master Google Ads Specialist Certificate successfully generated and added to your verified profile!');
      setTimeout(() => {
  ...
  const getLevelBadgeText = (lvl: CourseLevel) => {
    switch (lvl) {
      case 'beginner':
        return 'Level 1: Beginner • 12 Modules + Local Lead Gen Project + Assessment (20 MCQs)';
      case 'intermediate':
        return 'Level 2: Intermediate • 14 Modules + E-commerce Growth Project + Assessment (30 MCQs)';
      case 'advanced':
  ...
  return (
    <div className="relative min-h-screen bg-[color:var(--color-bg-base)] pb-24 lg:pb-8 overflow-hidden">
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-10">
        ...
        <CourseSwitcherNav
          currentCourseTitle="Google Ads Masterclass"
          currentCourseId="google-ads"
        />

        {/* Master Certificate Unlocked Hero Banner */}
        {certEligibility.isEligible && (
          <div className="mb-8 p-6 rounded-2xl bg-gradient-to-r from-blue-600/20 via-indigo-600/20 to-sky-600/20 border-2 border-blue-400/50 shadow-xl flex flex-col sm:flex-row items-center justify-between gap-6 animate-in fade-in zoom-in-95 duration-300">
            <div className="flex items-center gap-4">
              <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-blue-500 to-indigo-600 flex items-center justify-center text-white shadow-lg flex-shrink-0">
              ...
              <div>
                <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full bg-blue-400/20 text-blue-300 border border-blue-400/30 text-xs font-mono font-bold uppercase tracking-wider mb-1">
                  <CheckBadgeIcon className="w-3.5 h-3.5" /> Master Google Ads Certified
                </span>
                <h2 className="text-xl font-heading font-bold text-[color:var(--text-primary)]">
                  Google Ads Masterclass Completed!
                </h2>
            ...
            <Button
              ...
              className="w-full sm:w-auto flex items-center justify-center gap-2 whitespace-nowrap bg-gradient-to-r from-blue-500 to-indigo-600 hover:from-blue-600 hover:to-indigo-700 text-white font-bold cursor-pointer shadow-lg shadow-blue-500/20"
            >
          ...
          <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
            <div>
              <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-blue-500/10 border border-blue-500/20 text-blue-600 dark:text-blue-400 text-xs font-mono font-bold uppercase tracking-wider mb-2">
                <MegaphoneIcon className="w-4 h-4" /> Paid Search &amp; Performance Marketing Track
              </div>
              ...
              <p className="text-sm sm:text-base text-[color:var(--text-secondary)] mt-1.5 leading-relaxed">
                A complete practical masterclass teaching Google Paid Search, auction mechanics, conversion tracking, GTM, GA4, Smart Bidding, Performance Max, scripts automation, and enterprise scaling with 3 formal assessments.
              </p>
            </div>
