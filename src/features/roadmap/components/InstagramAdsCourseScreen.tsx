...
export const InstagramAdsCourseScreen: React.FC = () => {
  const [courseState, setCourseState] = useState<CourseProgressState>(() => InstagramAdsCourseService.getCourseState());
  ...
  const loadState = useCallback(() => {
    const state = InstagramAdsCourseService.getCourseState();
    ...
    if (levelParam && ['beginner', 'intermediate', 'advanced'].includes(levelParam)) {
      const check = InstagramAdsCourseService.checkLevelPrerequisites(levelParam, state);
      if (!check.canUnlock) {
      ...
      const activeCheck = InstagramAdsCourseService.checkLevelPrerequisites(state.currentLevel, state);
  ...
  const certEligibility = InstagramAdsCourseService.isEligibleForMasterCertificate(courseState);
  ...
  const handleLevelSelect = (level: CourseLevel) => {
    const res = InstagramAdsCourseService.switchLevel(level);
    if (res.success) {
  ...
  const handleTaskCompleted = async (taskId: string) => {
    const updatedState = InstagramAdsCourseService.completeTask(activeLevel, taskId);
    setCourseState(updatedState);

    // Auto-check if completed master level to issue credential
    if (user?.id) {
      const eligibility = InstagramAdsCourseService.isEligibleForMasterCertificate(updatedState);
      if (eligibility.isEligible) {
        try {
          await CertificateService.getOrCreateCertificate(
            user.id,
            'Instagram Ads Masterclass — Visual Performance Media Specialist',
            '3d-gold'
          );
        } catch (e) {
  ...
  const handleClaimCertificate = async () => {
    ...
    try {
      ...
      setCertClaimedToast('🎉 Master Instagram Ads Specialist Certificate successfully generated and added to your verified profile!');
      setTimeout(() => {
  ...
  const getLevelBadgeText = (lvl: CourseLevel) => {
    switch (lvl) {
      case 'beginner':
        return 'Level 1: Beginner • 12 Modules + DR E-Commerce Launch Project + Assessment (20 MCQs)';
      case 'intermediate':
        return 'Level 2: Intermediate • 14 Modules + Full-Funnel Scaling Project + Assessment (30 MCQs)';
      case 'advanced':
  ...
  return (
    <div className="relative min-h-screen bg-[color:var(--color-bg-base)] pb-24 lg:pb-8 overflow-hidden">
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-10">
        ...
        <CourseSwitcherNav
          currentCourseTitle="Instagram Ads Masterclass"
          currentCourseId="instagram-ads"
        />

        {/* Master Certificate Unlocked Hero Banner */}
        {certEligibility.isEligible && (
          <div className="mb-8 p-6 rounded-2xl bg-gradient-to-r from-pink-600/20 via-purple-600/20 to-amber-600/20 border-2 border-pink-400/50 shadow-xl flex flex-col sm:flex-row items-center justify-between gap-6 animate-in fade-in zoom-in-95 duration-300">
            <div className="flex items-center gap-4">
              <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-pink-500 via-rose-500 to-amber-500 flex items-center justify-center text-white shadow-lg flex-shrink-0">
                <TrophyIcon className="w-8 h-8" />
              </div>
              <div>
                <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full bg-pink-400/20 text-pink-300 border border-pink-400/30 text-xs font-mono font-bold uppercase tracking-wider mb-1">
                  <CheckBadgeIcon className="w-3.5 h-3.5" /> Master Instagram Ads Certified
                </span>
                <h2 className="text-xl font-heading font-bold text-[color:var(--text-primary)]">
                  Instagram Ads Masterclass Completed!
                </h2>
            ...
            <Button
              ...
              className="w-full sm:w-auto flex items-center justify-center gap-2 whitespace-nowrap bg-gradient-to-r from-pink-500 via-purple-600 to-rose-600 hover:from-pink-600 hover:to-rose-700 text-white font-bold cursor-pointer shadow-lg shadow-pink-500/20"
            >
        ...
        <header className="mb-8 space-y-4">
          <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
            <div>
              <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-pink-500/10 border border-pink-500/20 text-pink-600 dark:text-pink-400 text-xs font-mono font-bold uppercase tracking-wider mb-2">
                <DevicePhoneMobileIcon className="w-4 h-4" /> Visual Discovery &amp; Social Commerce Track
              </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 pt-2">
            ...
            <div className="p-3 rounded-xl bg-[color:var(--color-bg-card)] border border-border">
              ...
              <p className="text-base font-heading font-bold text-pink-500">Master Certificate</p>
            </div>
          </div>
