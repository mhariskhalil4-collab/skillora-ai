...
export const SeoCourseScreen: React.FC = () => {
  const [courseState, setCourseState] = useState<CourseProgressState>(() => SeoCourseService.getCourseState());
  ...
  const loadState = useCallback(() => {
    const state = SeoCourseService.getCourseState();
    ...
    if (levelParam && ['beginner', 'intermediate', 'advanced'].includes(levelParam)) {
      const check = SeoCourseService.checkLevelPrerequisites(levelParam, state);
      if (!check.canUnlock) {
  ...
  const certEligibility = SeoCourseService.isEligibleForMasterCertificate(courseState);
  ...
  const handleLevelSelect = (level: CourseLevel) => {
    const res = SeoCourseService.switchLevel(level);
    if (res.success) {
  ...
  const handleTaskCompleted = async (taskId: string) => {
    const updatedState = SeoCourseService.completeTask(activeLevel, taskId);
    setCourseState(updatedState);

    // Auto-check if completed master level to issue credential
    if (user?.id) {
      const eligibility = SeoCourseService.isEligibleForMasterCertificate(updatedState);
  ...
  const handleClaimCertificate = async () => {
    ...
    try {
      ...
      setCertClaimedToast('🎉 Master SEO Professional Certificate successfully generated and added to your verified profile!');
      setTimeout(() => {
  ...
  return (
    <div className="relative min-h-screen bg-[color:var(--color-bg-base)] pb-24 lg:pb-8 overflow-hidden">
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-10">
        ...
        <CourseSwitcherNav
          currentCourseTitle="SEO Masterclass"
          currentCourseId="seo"
        />
        ...
        {certEligibility.isEligible && (
          <div className="mb-8 p-6 rounded-2xl bg-gradient-to-r from-emerald-600/20 via-teal-600/20 to-cyan-600/20 border-2 border-emerald-400/50 shadow-xl flex flex-col sm:flex-row items-center justify-between gap-6 animate-in fade-in zoom-in-95 duration-300">
            <div className="flex items-center gap-4">
              <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-emerald-500 to-teal-700 flex items-center justify-center text-white shadow-lg flex-shrink-0">
              ...
              <div>
                <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full bg-emerald-400/20 text-emerald-300 border border-emerald-400/30 text-xs font-mono font-bold uppercase tracking-wider mb-1">
                  <CheckBadgeIcon className="w-3.5 h-3.5" /> Master SEO Certified Professional
                </span>
                <h2 className="text-xl font-heading font-bold text-[color:var(--text-primary)]">
                  SEO Masterclass Completed!
                </h2>
            ...
            <Button
              ...
              className="w-full sm:w-auto flex items-center justify-center gap-2 whitespace-nowrap bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-700 hover:to-teal-700 text-white font-bold cursor-pointer shadow-lg shadow-emerald-500/20"
            >
          ...
          <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
            <div>
              <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-emerald-500/10 border border-emerald-500/20 text-emerald-600 dark:text-emerald-400 text-xs font-mono font-bold uppercase tracking-wider mb-2">
                <AcademicCapIcon className="w-4 h-4" /> Search Engine Optimization Track
              </div>
              ...
              <p className="text-sm sm:text-base text-[color:var(--text-secondary)] mt-1.5 leading-relaxed">
                Master complete professional SEO from core crawling, keyword intent, and on-page mechanics to Core Web Vitals, JSON-LD structured data, e-commerce facets, international hreflang, Python log analysis, and enterprise crisis recovery.
              </p>
            </div>
