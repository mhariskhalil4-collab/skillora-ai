...
export const PromptEngineeringCourseScreen: React.FC = () => {
  const [courseState, setCourseState] = useState<CourseProgressState>(() => PromptEngineeringCourseService.getCourseState());
  ...
  const loadState = useCallback(() => {
    ...
    if (levelParam && ['beginner', 'intermediate', 'advanced'].includes(levelParam)) {
      const check = PromptEngineeringCourseService.checkLevelPrerequisites(levelParam, state);
      if (!check.canUnlock) {
  ...
  const certEligibility = PromptEngineeringCourseService.isEligibleForMasterCertificate(courseState);
  ...
  const handleLevelSelect = (level: CourseLevel) => {
    const res = PromptEngineeringCourseService.switchLevel(level);
    if (res.success) {
  ...
  const handleTaskCompleted = async (taskId: string) => {
    const updatedState = PromptEngineeringCourseService.completeTask(activeLevel, taskId);
    setCourseState(updatedState);

    // Auto-check if completed master level to issue credential
    if (user?.id) {
      const eligibility = PromptEngineeringCourseService.isEligibleForMasterCertificate(updatedState);
      if (eligibility.isEligible) {
        try {
          await CertificateService.getOrCreateCertificate(
            user.id,
            'Prompt Engineering — AI Prompt Engineering Masterclass',
            '3d-gold'
          );
        } catch (e) {
  ...
  const handleClaimCertificate = async () => {
    ...
    try {
      ...
      setCertClaimedToast('🎉 Master AI Prompt Engineering Certificate successfully generated and added to your verified profile!');
      setTimeout(() => {
  ...
  return (
    <div className="relative min-h-screen bg-[color:var(--color-bg-base)] pb-24 lg:pb-8 overflow-hidden">
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-10">
        ...
        <CourseSwitcherNav
          currentCourseTitle="Prompt Engineering Masterclass"
          currentCourseId="prompt-engineering"
        />

        {/* Master Certificate Unlocked Hero Banner */}
        {certEligibility.isEligible && (
          <div className="mb-8 p-6 rounded-2xl bg-gradient-to-r from-purple-600/20 via-indigo-600/20 to-blue-600/20 border-2 border-purple-400/50 shadow-xl flex flex-col sm:flex-row items-center justify-between gap-6 animate-in fade-in zoom-in-95 duration-300">
            <div className="flex items-center gap-4">
              <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-purple-500 to-indigo-700 flex items-center justify-center text-white shadow-lg flex-shrink-0">
              ...
              <div>
                <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full bg-purple-400/20 text-purple-300 border border-purple-400/30 text-xs font-mono font-bold uppercase tracking-wider mb-1">
                  <CheckBadgeIcon className="w-3.5 h-3.5" /> Master Prompt Engineering Certified
                </span>
                <h2 className="text-xl font-heading font-bold text-[color:var(--text-primary)]">
                  Prompt Engineering Masterclass Completed!
                </h2>
            ...
            <Button
              ...
              className="w-full sm:w-auto flex items-center justify-center gap-2 whitespace-nowrap bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-700 hover:to-indigo-700 text-white font-bold cursor-pointer shadow-lg shadow-purple-500/20"
            >
        ...
        <header className="mb-8 space-y-4">
          <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
            <div>
              <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-purple-500/10 border border-purple-500/20 text-purple-600 dark:text-purple-400 text-xs font-mono font-bold uppercase tracking-wider mb-2">
                <AcademicCapIcon className="w-4 h-4" /> AI, Automation &amp; Productivity Track
              </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 pt-2">
            ...
            <div className="p-3 rounded-xl bg-[color:var(--color-bg-card)] border border-border">
              ...
              <p className="text-base font-heading font-bold text-purple-500">Master Certificate</p>
            </div>
          </div>
