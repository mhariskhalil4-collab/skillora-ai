...
export const HtmlCssCourseScreen: React.FC = () => {
  const [courseState, setCourseState] = useState<CourseProgressState>(() => HtmlCssCourseService.getCourseState());
  ...
  const loadState = useCallback(() => {
    ...
    if (levelParam && ['beginner', 'intermediate', 'advanced'].includes(levelParam)) {
      const check = HtmlCssCourseService.checkLevelPrerequisites(levelParam, state);
      if (!check.canUnlock) {
  ...
  const certEligibility = HtmlCssCourseService.isEligibleForMasterCertificate(courseState);
  ...
  const handleLevelSelect = (level: CourseLevel) => {
    const res = HtmlCssCourseService.switchLevel(level);
    if (res.success) {
  ...
  const handleTaskCompleted = async (taskId: string) => {
    const updatedState = HtmlCssCourseService.completeTask(activeLevel, taskId);
    setCourseState(updatedState);

    // Auto-check if completed master level to issue credential
    if (user?.id) {
      const eligibility = HtmlCssCourseService.isEligibleForMasterCertificate(updatedState);
      if (eligibility.isEligible) {
        try {
          await CertificateService.getOrCreateCertificate(
            user.id,
            'HTML & CSS Masterclass — From Beginner to Professional Web Developer',
            '3d-gold'
          );
        } catch (e) {
  ...
  const handleClaimCertificate = async () => {
    ...
    try {
      ...
      setCertClaimedToast('🎉 Master Web Developer Certificate successfully generated and added to your verified profile!');
      setTimeout(() => {
  ...
  const getLevelBadgeText = (lvl: CourseLevel) => {
    switch (lvl) {
      case 'beginner':
        return 'Level 1: Beginner • 12 Modules + Capstone Project + Assessment (20 MCQs)';
      case 'intermediate':
        return 'Level 2: Intermediate • 14 Modules + Capstone Project + Assessment (30 MCQs)';
      case 'advanced':
        return 'Level 3: Advanced • 14 Modules + 2 Production Projects + Master Assessment (40 MCQs)';
    }
  };
  ...
  return (
    <div className="relative min-h-screen bg-[color:var(--color-bg-base)] pb-24 lg:pb-8 overflow-hidden">
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-10">
        ...
        <CourseSwitcherNav
          currentCourseTitle="HTML & CSS Masterclass"
          currentCourseId="html-css"
        />

        {/* Master Certificate Unlocked Hero Banner */}
        {certEligibility.isEligible && (
          <div className="mb-8 p-6 rounded-2xl bg-gradient-to-r from-orange-600/20 via-amber-600/20 to-blue-600/20 border-2 border-orange-400/50 shadow-xl flex flex-col sm:flex-row items-center justify-between gap-6 animate-in fade-in zoom-in-95 duration-300">
            <div className="flex items-center gap-4">
              <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-orange-500 to-amber-700 flex items-center justify-center text-white shadow-lg flex-shrink-0">
                <TrophyIcon className="w-8 h-8" />
              </div>
              <div>
                <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full bg-orange-400/20 text-orange-300 border border-orange-400/30 text-xs font-mono font-bold uppercase tracking-wider mb-1">
                  <CheckBadgeIcon className="w-3.5 h-3.5" /> Master Web Developer Certified
                </span>
                <h2 className="text-xl font-heading font-bold text-[color:var(--text-primary)]">
                  HTML &amp; CSS Masterclass Completed!
                </h2>
            ...
            <Button
              ...
              className="w-full sm:w-auto flex items-center justify-center gap-2 whitespace-nowrap bg-gradient-to-r from-orange-600 to-amber-600 hover:from-orange-700 hover:to-amber-700 text-white font-bold cursor-pointer shadow-lg shadow-orange-500/20"
            >
        ...
        <header className="mb-8 space-y-4">
          <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
            <div>
              <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-orange-500/10 border border-orange-500/20 text-orange-600 dark:text-orange-400 text-xs font-mono font-bold uppercase tracking-wider mb-2">
                <CodeBracketIcon className="w-4 h-4" /> Web Development Track
              </div>
              ...
              <p className="text-sm sm:text-base text-[color:var(--text-secondary)] mt-1.5 leading-relaxed">
                A complete practical course teaching learners how to build semantic, accessible, responsive, and high-performance modern web applications from scratch, covering HTML5 landmarks, CSS Grid, Flexbox, Container Queries, Cascade Layers, and enterprise design systems.
              </p>
            </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 pt-2">
            ...
            <div className="p-3 rounded-xl bg-[color:var(--color-bg-card)] border border-border">
              ...
              <p className="text-base font-heading font-bold text-orange-500">Master Certificate</p>
            </div>
          </div>
