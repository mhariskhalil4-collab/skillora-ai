...
export const FacebookAdsCourseScreen: React.FC = () => {
  const [courseState, setCourseState] = useState<CourseProgressState>(() => FacebookAdsCourseService.getCourseState());
  ...
  const loadState = useCallback(() => {
    const state = FacebookAdsCourseService.getCourseState();
    ...
    if (levelParam && ['beginner', 'intermediate', 'advanced'].includes(levelParam)) {
      const check = FacebookAdsCourseService.checkLevelPrerequisites(levelParam, state);
      if (!check.canUnlock) {
      ...
      const activeCheck = FacebookAdsCourseService.checkLevelPrerequisites(state.currentLevel, state);
  ...
  const certEligibility = FacebookAdsCourseService.isEligibleForMasterCertificate(courseState);
  ...
  const handleLevelSelect = (level: CourseLevel) => {
    const res = FacebookAdsCourseService.switchLevel(level);
    if (res.success) {
  ...
  const handleTaskCompleted = async (taskId: string) => {
    const updatedState = FacebookAdsCourseService.completeTask(activeLevel, taskId);
    setCourseState(updatedState);

    // Auto-check if completed master level to issue credential
    if (user?.id) {
      const eligibility = FacebookAdsCourseService.isEligibleForMasterCertificate(updatedState);
      if (eligibility.isEligible) {
        try {
          await CertificateService.getOrCreateCertificate(
            user.id,
            'Facebook Ads Masterclass — Meta Performance Marketing & Media Buying Specialist',
            '3d-gold'
          );
        } catch (e) {
  ...
  const handleClaimCertificate = async () => {
    ...
    try {
      ...
      setCertClaimedToast('🎉 Master Facebook Ads Specialist Certificate successfully generated and added to your verified profile!');
      setTimeout(() => {
  ...
  return (
    <div className="relative min-h-screen bg-[color:var(--color-bg-base)] pb-24 lg:pb-8 overflow-hidden">
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-10">
        ...
        <CourseSwitcherNav
          currentCourseTitle="Facebook Ads Masterclass"
          currentCourseId="facebook-ads"
        />
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full bg-blue-400/20 text-blue-300 border border-blue-400/30 text-xs font-mono font-bold uppercase tracking-wider mb-1">
                  <CheckBadgeIcon className="w-3.5 h-3.5" /> Master Facebook Ads Certified
                </span>
                <h2 className="text-xl font-heading font-bold text-[color:var(--text-primary)]">
                  Facebook Ads Masterclass Completed!
                </h2>
          ...
          <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
            <div>
              <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-blue-500/10 border border-blue-500/20 text-blue-600 dark:text-blue-400 text-xs font-mono font-bold uppercase tracking-wider mb-2">
                <MegaphoneIcon className="w-4 h-4" /> Meta Ads &amp; Paid Social Performance Track
              </div>
